...
final class GunBoundTests: XCTestCase {
    ...
    func testGameResultCommand() {
        ...
        let data = Data([0x46, 0x00, 0xDC, 0x0E, 0x12, 0x44, 0x9E, 0xEE, 0xC7, 0x7E, 0x68, 0x00, 0x62, 0x0D, 0x03, 0x19, 0xE1, 0x02, 0x7F, 0x1C, 0x97, 0x1C, 0xA8, 0xB3, 0x38, 0xE9, 0x5D, 0x36, 0xC8, 0x3C, 0x8A, 0x84, 0x49, 0x6E, 0xFA, 0x78, 0xAD, 0x45, 0xB3, 0x46, 0xDD, 0x71, 0x58, 0x10, 0xB8, 0x27, 0xA8, 0x7D, 0xF4, 0x6F, 0xA1, 0x88, 0xF1, 0x24, 0x38, 0x4F, 0xD9, 0xB7, 0xA2, 0x4A, 0x89, 0x09, 0x23, 0x9D, 0x72, 0xFF, 0x11, 0x33, 0xC0, 0x2F])
        ...
        XCTAssertEqual(packet.data, data)
        XCTAssertEqual(packet.size, 70)
        XCTAssertEqual(packet.size, numericCast(packet.data.count))
        XCTAssertEqual(packet.opcode, .playResultCommand)
        XCTAssertEqual(packet.opcode.type, .command)
        XCTAssertEqual(packet.id, 0x0EDC)
