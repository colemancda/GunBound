...
/// original binary). `decompress(compress(x), decodedSize: x.count) == x`
/// proves the encoder's LZSS matches, position codes, and adaptive-Huffman
/// tree stay in lock-step with the decoder; the reference-fixture test
/// additionally pins the exact output bytes against the original `LZHUF.C`
/// encoder.
@Suite struct LZHUFCompressTests {
    ...
    /// header the classic CLI writes and GunBound's containers don't).
