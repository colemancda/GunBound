...
/// (`ImgFile.Pixel`), so no image-file loader (`SDL3Image`) is needed here —
/// this just repacks each pixel into a native-endian `ARGB8888` word (as
/// `SDL_PIXELFORMAT_ARGB8888` expects) and uploads it directly, the same
/// raw-pixel-upload technique the SDL3 port's `solidTexture` uses.
enum FrameTexture {
