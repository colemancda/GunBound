...
internal struct Crypto {
    ...
    enum AES {
        ...
        static func decrypt(_ data: Data, key: Key, opcode: Opcode) throws -> Data {
            let checksum = 0x8631_607E + UInt32(opcode.rawValue)
    ...
    enum SHA0 {
        ...
        static func sha0_process_block(_ chunk: [UInt8]) -> [UInt32] {
            ...
            var sha_h: [UInt32] = [0x6745_2301, 0xEFCD_AB89, 0x98BA_DCFE, 0x1032_5476, 0xC3D2_E1F0]
            ...
            sha_h[0] = UInt32((UInt64(sha_h[0]) + UInt64(a)) & 0xffff_ffff)
            sha_h[1] = UInt32((UInt64(sha_h[1]) + UInt64(b)) & 0xffff_ffff)
            sha_h[2] = UInt32((UInt64(sha_h[2]) + UInt64(c)) & 0xffff_ffff)
            sha_h[3] = UInt32((UInt64(sha_h[3]) + UInt64(d)) & 0xffff_ffff)
            sha_h[4] = UInt32((UInt64(sha_h[4]) + UInt64(e)) & 0xffff_ffff)
