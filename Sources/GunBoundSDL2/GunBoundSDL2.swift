...
@main
struct GunBoundSDL2: ParsableCommand {

    static let configuration = CommandConfiguration(
        commandName: "GunBoundSDL2",
        abstract: "Native SDL2 GunBound client"
    )
...
func runClient(assetsDirectory: URL, fullscreen: Bool, network: NetworkConfig) throws {
    ...
    defer { SDL.quit() }
    try SDL.initializeMixer(formats: [.mp3])
    defer { SDL.quitMixer() }
    try SDL.openAudio()
    defer { SDL.closeAudio() }

    var windowOptions: BitMaskOptionSet<SDLWindow.Option> = []
    ...
    try sdlRenderer.setLogicalSize(width: windowWidth, height: windowHeight)
    ...
    let renderer = SDL2Renderer(renderer: sdlRenderer)
    ...
    let context = ClientContext(assets: assets, renderer: renderer, network: network) {
        SDL2AudioPlayer(coordinator: music)
    }

    // Decomp-confirmed normal boot path: InitGame -> ChangeGameState(6) =
    // Logo2 -> Title -> ServerSelect. Logo1(5) is an InitGame-failure
    // fallback only (ARCHITECTURE.md "Screen flow"), not part of a normal
    // cold boot, so it's reachable but not the entry point.
        ...
        while let event = SDL.pollEvent() {
            ...
            case .keyDown(let scancode, _)
                where scancode.rawValue == SDL_SCANCODE_Q.rawValue
                    && (SDL_GetModState().rawValue & (KMOD_LGUI.rawValue | KMOD_RGUI.rawValue)) != 0:
                running = false
            default:
